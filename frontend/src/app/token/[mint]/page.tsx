...
const TopHolders = dynamic(() => import("@/components/TopHolders").then((m) => m.TopHolders), {
  loading: () => <div className="flex items-center justify-center h-32 text-text-muted text-xs">Loading holders...</div>,
});
const BundleChecker = dynamic(() => import("@/components/BundleChecker").then((m) => m.BundleChecker), {
  loading: () => <div className="flex items-center justify-center h-32 text-text-muted text-xs">Loading bundles...</div>,
});
...
export default function TokenPage() {
  ...
  const { data: tokenInfo, isLoading: tokenLoading, isFetched: tokenFetched } = useQuery({
    ...
    enabled: !!mint,
    refetchInterval: 15_000 * visScale,
    staleTime: 8_000,
  ...
  const chartRefetchInterval = (() => {
    switch (range) {
      case "1s": return 10_000;
      case "5s": return 15_000;
      case "15s": return 20_000;
      case "30s": return 30_000;
      case "1m": return 40_000;
      case "5m": return 60_000;
      case "15m": return 120_000;
  ...
  const { data: chartData, isLoading: chartLoading } = useQuery({
    ...
    staleTime: isShortRange ? 4_000 : 20_000,
    placeholderData: keepPreviousData,
    retry: 2,
  });
  ...
  useEffect(() => {
    if (mint) {
      // Only prefetch trades (visible by default). Holders/bundles load on tab switch.
  ...
  // Adjacent chart prefetching removed to reduce API calls

  // Debounced WS price → chart updates: batch into 200ms windows to reduce re-renders
  const pendingPriceRef = useRef<{ price: number; ts: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const flushPrice = () => {
      ...
      const nowSec = Math.floor(pending.ts / 1000);
      const currentBucket = Math.floor(nowSec / bucketSec) * bucketSec;

      queryClient.setQueryData<{ bars: import("@/lib/api").OHLCVBar[] }>(["chart", mint, range], (old) => {
        if (!old || !old.bars || old.bars.length === 0) return old;
        const updated = [...old.bars];
    ...
    const unsub = wsClient.on("price", (msg) => {
      ...
      pendingPriceRef.current = { price, ts: Date.now() };
  ...
  const avgExitPrice = useMemo(() => {
    ...
    // If position is currently open, show avg sell from partial sells in current cycle
    if (position && position.qty > 0) {
      const currentCycleTrades = mintTrades.slice(lastFullExitIdx + 1);
      const sellTrades = currentCycleTrades.filter((tr) => tr.side === "sell");
  ...
  // Compute realized dollar PnL for the last closed cycle
  const realizedPnlDollar = useMemo(() => {
    if (!userTradesData?.trades) return undefined;
    const mintTrades = userTradesData.trades
      .filter((tr) => tr.mint === mint)
      .sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    ...
    // For open position, compute partial realized PnL from current cycle sells vs buys
    if (position && position.qty > 0) {
      const currentCycleTrades = mintTrades.slice(lastFullExitIdx + 1);
      const buys = currentCycleTrades.filter((tr) => tr.side === "buy");
      const sells = currentCycleTrades.filter((tr) => tr.side === "sell");
      if (sells.length === 0) return undefined;
      const totalBuyCost = buys.reduce((sum, tr) => sum + tr.qty * tr.price, 0);
      const totalSellProceeds = sells.reduce((sum, tr) => sum + tr.qty * tr.price, 0);
      const totalBuyQty = buys.reduce((sum, tr) => sum + tr.qty, 0);
      const totalSellQty = sells.reduce((sum, tr) => sum + tr.qty, 0);
      // Proportional cost of sold quantity
      const avgBuyPrice = totalBuyQty > 0 ? totalBuyCost / totalBuyQty : 0;
      return totalSellProceeds - (avgBuyPrice * totalSellQty);
    }

    // For closed position, compute PnL from last completed cycle
    const cycleStart = prevFullExitIdx + 1;
    const cycleEnd = lastFullExitIdx + 1;
    const cycleTrades = mintTrades.slice(cycleStart, cycleEnd);
    const buys = cycleTrades.filter((tr) => tr.side === "buy");
    const sells = cycleTrades.filter((tr) => tr.side === "sell");
    if (buys.length === 0 || sells.length === 0) return undefined;
    const totalBuyCost = buys.reduce((sum, tr) => sum + tr.qty * tr.price, 0);
    const totalSellProceeds = sells.reduce((sum, tr) => sum + tr.qty * tr.price, 0);
    ...
    return (
      <div className="pt-3 pb-6 page-enter">
        <div className="flex items-center gap-3 mb-3 pb-3 border-b border-white/10">
          <div className="h-9 w-9 rounded-xl skeleton-shimmer ring-1 ring-white/10" />
          <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-3">
          <div className="flex flex-col gap-3">
            <div className="rounded-xl border border-white/10 bg-black/35 h-[480px] skeleton-shimmer" />
            <div className="rounded-xl border border-white/10 bg-black/35 h-[300px] skeleton-shimmer" />
          </div>
          <div className="flex flex-col gap-3">
            <div className="rounded-xl border border-white/10 bg-black/35 h-[300px] skeleton-shimmer" />
            <div className="rounded-xl border border-white/10 bg-black/35 h-[220px] skeleton-shimmer" />
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex flex-wrap items-center gap-3 mb-3 pb-3 border-b border-white/10">
        <div className="flex items-center gap-2.5">
          {t.image ? (
            ...
            <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-[#39FF14]/20 to-[#4fc3f7]/10 flex items-center justify-center text-sm font-bold text-[#39FF14]/60 ring-1 ring-white/10">
              {t.symbol?.charAt(0)}
            </div>
          )}
          <div>
        ...
        <div className="flex items-center gap-3 ml-auto">
          ...
          <VolumeToggle
            period={volumePeriod}
            onToggle={() => {
              const idx = VOLUME_PERIODS.indexOf(volumePeriod);
              setVolumePeriod(VOLUME_PERIODS[(idx + 1) % VOLUME_PERIODS.length]);
            }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-3">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-xl border border-white/10 bg-black/35 backdrop-blur overflow-hidden">
            {/* Timeframe bar */}
            <div className="flex items-center gap-0.5 px-3 py-2 border-b border-white/10 bg-white/[0.03] overflow-x-auto">
              {ALL_RANGES.map((r) => (
                <button
                  key={r}
                  onClick={() => setRange(r)}
            ...
            <PositionBanner
              ...
              apiPrice={t.price}
              realizedPnlDollar={realizedPnlDollar}
              chartMode={chartMode}
              supply={t.supply}
              marketCap={t.marketCap}
            />
            {chartLoading && chartBars.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-[400px] text-white/50 bg-transparent gap-3">
              ...
              <>
                ...
                <Chart data={chartBars} height={400} supply={t.supply} marketCap={t.marketCap} currentPrice={t.price} range={usingFallback ? fallbackRange! : range} avgEntryPrice={effectiveAvgEntry} avgExitPrice={avgExitPrice} onModeChange={setChartMode} />
              </>
            ) : (
              <div className="flex flex-col items-center justify-center h-[400px] text-white/50 bg-transparent gap-2">
        ...
        <div className="flex flex-col gap-3">
          <OrderPanel token={t} usdcBalance={usdcBalance} tokenQty={tokenQty} avgEntryPrice={effectiveAvgEntry} portfolioLoading={portfolioLoading} />
...
function PositionBanner({
  ...
  // Direct WS-driven price updates for instant PnL
  useEffect(() => {
    const unsub = wsClient.on("price", (msg) => {
      if (msg.mint !== mint) return;
  ...
  // Backup interval at 100ms for smooth PnL ticking
  useEffect(() => {
  ...
  // Compute mcap multiplier for converting price to mcap
  const mcapMul = (() => {
    if (!isMcap) return 1;
    if (supply && supply > 0) return supply;
    if (marketCap && apiPrice && apiPrice > 0) return marketCap / apiPrice;
    return 1;
  })();

  const formatVal = (price: number) => {
    if (isMcap) return formatCompact(price * mcapMul);
    return formatPrice(price);
  };

  const valLabel = isMcap ? "MCap" : "Price";
  ...
  if (hasOpenPosition) {
    ...
    return (
      <div className="flex items-center gap-4 px-3 py-2 bg-white/[0.03] border-b border-white/10 text-[10px]">
        <span className="text-white/40 font-semibold uppercase tracking-wider text-[8px]">My Position</span>
        <span className="font-mono text-text-secondary">Qty: <b className="text-text-primary">{formatNumber(position.qty, 4)}</b></span>
        <span className="font-mono text-text-secondary">Bought @{valLabel}: <b className="text-text-primary">{formatVal(entry)}</b></span>
        {avgExitPrice && avgExitPrice > 0 && (
          <span className="font-mono text-text-secondary">Sold @{valLabel}: <b className="text-accent-orange">{formatVal(avgExitPrice)}</b></span>
        )}
  ...
  if (hasTrades && avgEntryFromTrades && avgExitPrice) {
    ...
    return (
      <div className="flex items-center gap-4 px-3 py-2 bg-white/[0.03] border-b border-white/10 text-[10px]">
        <span className="text-white/40 font-semibold uppercase tracking-wider text-[8px]">Closed Position</span>
        <span className="font-mono text-text-secondary">Bought @{valLabel}: <b className="text-text-primary">{formatVal(avgEntryFromTrades)}</b></span>
        <span className="font-mono text-text-secondary">Sold @{valLabel}: <b className="text-text-primary">{formatVal(avgExitPrice)}</b></span>
        <span className={`font-mono font-bold ${realizedRoi >= 0 ? "text-accent-green" : "text-accent-red"}`}>
          Return: {formatPercent(realizedRoi)}
        </span>
        {realizedPnlDollar !== undefined && (
          <span className={`font-mono font-bold ${realizedPnlDollar >= 0 ? "text-accent-green" : "text-accent-red"}`}>
            {realizedPnlDollar >= 0 ? "+" : ""}{formatPnl(realizedPnlDollar)}
          </span>
        )}
      </div>
    );
...
function VolumeToggle({ period, onToggle, volume24h, chartBars }: {
  period: VolumePeriod;
  onToggle: () => void;
  volume24h: number;
  chartBars: import("@/lib/api").OHLCVBar[];
}) {
  const volume = useMemo(() => {
    if (period === "24h") return volume24h;
    // Calculate volume from chart bars for shorter periods
    const now = Math.floor(Date.now() / 1000);
    const periodSec: Record<string, number> = { "5m": 300, "1h": 3600, "6h": 21600 };
    const cutoff = now - (periodSec[period] || 86400);
    const recentBars = chartBars.filter((b) => b.time >= cutoff);
    const chartVol = recentBars.reduce((sum, b) => sum + (b.volume || 0), 0);
    // Always show a value: fall back to estimated volume from 24h if chart has no data
    if (chartVol > 0) return chartVol;
    const scaleMap: Record<string, number> = { "5m": 5 / 1440, "1h": 1 / 24, "6h": 6 / 24 };
    return volume24h * (scaleMap[period] || 1);
  }, [period, volume24h, chartBars]);

  return (
    <button onClick={onToggle} className="flex flex-col items-center cursor-pointer hover:opacity-80 transition-opacity" title="Click to change volume period">
      <span className="text-[8px] text-text-muted uppercase tracking-wider font-semibold flex items-center gap-0.5">
        {period} Vol
        <svg className="w-2 h-2 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 15L12 18.75 15.75 15m-7.5-6L12 5.25 15.75 9" />
        </svg>
      </span>
      <span className="text-[12px] font-mono font-bold text-text-primary">{formatCompact(volume)}</span>
    </button>
  );
}
