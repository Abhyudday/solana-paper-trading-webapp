  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <div className="relative w-16 h-9 overflow-hidden">
        <div className="absolute inset-0 rounded-t-full border-4 border-white/[0.06] border-b-0" />
        <div
  ...
  return (
    <div className="overflow-hidden">
      {isLoading ? (
        ...
        <div className="text-center text-white/40 text-[10px] py-8">No bundle data available</div>
      ) : (
        <>
          {/* Sub-tabs */}
            ...
            {(["overview", "bundles", "snipers"] as BundleTab[]).map((t) => (
              <button
                ...
                className={`flex-1 py-2 text-[9px] font-bold uppercase tracking-wider transition-all relative ${
                  tab === t
                    ? "text-[#39FF14] bg-white/[0.03]"
                    : "text-white/40 hover:text-white/60"
                }`}
              >
                {t === "overview" ? "Overview" : t === "bundles" ? `Bundles (${bundles.bundleCount})` : `Snipers (${sniperInfo?.sniperCount || 0})`}
                {tab === t && <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#39FF14]/50" />}
              </button>
            ))}
          </div>
          ...
          {tab === "overview" && (
            <div className="p-3">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex flex-col gap-2 text-right">
                  <div className="flex items-center gap-1.5 justify-end">
                    <span className="text-[8px] text-white/40 uppercase tracking-wider">Bundle</span>
                  ...
                  {sniperInfo && (
                    <div className="flex items-center gap-1.5 justify-end">
                      <span className="text-[8px] text-white/40 uppercase tracking-wider">Sniper</span>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex flex-col p-2 rounded-lg bg-white/[0.03] border border-white/[0.06]">
                  <span className="text-[7px] text-white/40 uppercase tracking-wider font-bold">Bundles</span>
                  <span className="text-[11px] font-mono font-bold text-white">{bundles.bundleCount}</span>
                </div>
                <div className="flex flex-col p-2 rounded-lg bg-white/[0.03] border border-white/[0.06]">
                  <span className="text-[7px] text-white/40 uppercase tracking-wider font-bold">Bundle %</span>
                  <span className={`text-[11px] font-mono font-bold ${bundles.bundlePercentage > 20 ? "text-accent-red" : bundles.bundlePercentage > 5 ? "text-accent-yellow" : "text-white"}`}>
                    {bundles.bundlePercentage.toFixed(2)}%
                  </span>
                  {bundles.initialPercentage > 0 && bundles.initialPercentage !== bundles.bundlePercentage && (
                    <span className="text-[8px] text-white/40 mt-0.5">Initial: {bundles.initialPercentage.toFixed(2)}%</span>
                  )}
                </div>
                <div className="flex flex-col p-2 rounded-lg bg-white/[0.03] border border-white/[0.06]">
                  <span className="text-[7px] text-white/40 uppercase tracking-wider font-bold">Snipers</span>
                  <span className="text-[11px] font-mono font-bold text-white">{sniperInfo?.sniperCount || 0}</span>
                </div>
                <div className="flex flex-col p-2 rounded-lg bg-white/[0.03] border border-white/[0.06]">
                  <span className="text-[7px] text-white/40 uppercase tracking-wider font-bold">Sniper %</span>
                  <span className={`text-[11px] font-mono font-bold ${(sniperInfo?.sniperPercentage || 0) > 10 ? "text-accent-red" : "text-white"}`}>
                    {(sniperInfo?.sniperPercentage || 0).toFixed(2)}%
                  </span>
                </div>
                {bundles.totalBalance > 0 && (
                  <div className="flex flex-col p-2 rounded-lg bg-white/[0.03] border border-white/[0.06] col-span-2">
                    <span className="text-[7px] text-white/40 uppercase tracking-wider font-bold">Total Bundled Balance</span>
                    <span className="text-[11px] font-mono font-bold text-white">{formatCompact(bundles.totalBalance)}</span>
                  </div>
                )}
              </div>
          ...
          {tab === "bundles" && (
            <div className="max-h-[200px] overflow-y-auto">
              {bundles.details.length === 0 ? (
                <div className="text-center text-white/40 text-[10px] py-8">No bundles detected</div>
              ) : (
                <>
                  <div className="flex items-center gap-2 px-3 py-1.5 text-[8px] text-white/40 uppercase tracking-widest font-bold border-b border-white/[0.06] sticky top-0 bg-black/60 backdrop-blur z-10">
                  ...
                  {bundles.details.map((detail, i) => (
                    <div key={i} className="dex-row flex items-center gap-2 px-3 py-1.5 text-[11px] font-mono border-b border-white/[0.06] group">
                      <div className="flex-1 flex items-center gap-1 min-w-0">
                        <a
                          href={`https://solscan.io/account/${detail.wallet}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-white/60 hover:text-[#4fc3f7] text-[10px] truncate transition-colors"
                          title={detail.wallet}
                        >
                      ...
                      <span className="w-[55px] text-right text-[10px] text-white/60">
                        {formatCompact(detail.balance)}
                      </span>
                      <span className={`w-[40px] text-right text-[10px] font-bold ${detail.percentage > 10 ? "text-accent-red" : "text-white/60"}`}>
                        {detail.percentage.toFixed(2)}%
                      </span>
                      <span className="w-[50px] text-right text-[9px] text-white/40">
                        {detail.bundleTime > 0
          ...
          {tab === "snipers" && (
            <div className="max-h-[200px] overflow-y-auto">
              {!sniperInfo || sniperInfo.snipers.length === 0 ? (
                ...
                <>
                  <div className="px-3 py-2 border-b border-white/[0.06] bg-accent-orange/5">
                  ...
                  {sniperInfo.snipers.map((sniper, i) => (
                    <div key={i} className="dex-row flex items-center gap-2 px-3 py-1.5 text-[11px] font-mono border-b border-white/[0.06] group">
                      ...
                      <span className="w-[50px] text-right text-[10px] text-white/60">
                        {formatCompact(sniper.amountUsd)}
                      </span>
                      <span className={`w-[35px] text-right text-[10px] font-bold ${sniper.percentage > 5 ? "text-accent-red" : "text-white/60"}`}>
                        {sniper.percentage.toFixed(1)}%
                      </span>
                      <span className="w-[35px] text-right text-[9px] text-white/40">
