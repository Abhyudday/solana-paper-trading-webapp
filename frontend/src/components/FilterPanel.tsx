...
export function FilterPanel({ onApply, onClose, initialFilters, columnType }: FilterPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-md bg-black/80 backdrop-blur border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-fade-in">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <h2 className="text-[13px] font-bold uppercase tracking-wider text-white">Filters</h2>
          <button
            onClick={onClose}
            className="text-white/40 hover:text-white h-6 w-6 rounded-lg flex items-center justify-center hover:bg-white/[0.06] transition-all"
          >
        ...
        <div className="px-5 py-4 max-h-[60vh] overflow-y-auto space-y-3">
          ...
          <div className="flex items-center gap-3">
            <span className="w-20 text-[11px] text-white/60 flex-shrink-0 font-semibold">Age</span>
            <div className="flex-1 flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2 text-[11px] text-white placeholder:text-white/30 focus:outline-none focus:border-[#39FF14]/30 focus:ring-1 focus:ring-[#39FF14]/10 pr-10"
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] text-white/40">min</span>
              </div>
              <div className="relative flex-1">
                <input
                  type="number"
          ...
          {FILTER_FIELDS.filter((field) => {
            ...
            <div key={field.label} className="flex items-center gap-3">
              <span className="w-20 text-[11px] text-white/60 flex-shrink-0 font-semibold">{field.label}</span>
              <div className="flex-1 flex gap-2">
                <div className="relative flex-1">
                  <input
                    type="number"
                  ...
                  />
                  {field.unit && (
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] text-white/40">
                      {field.unit}
                    </span>
                  )}
                </div>
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-t border-white/10">
          <button
            onClick={handleReset}
            className="text-[11px] text-white/40 hover:text-red-400 transition-colors font-semibold"
          >
            Reset All
          </button>
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              className="px-4 py-1.5 rounded-lg text-[11px] font-semibold bg-white/[0.04] text-white/60 hover:text-white border border-white/10 transition-all"
            >
              Cancel
            </button>
            <button
