...
function TxRow({ trade }: { trade: TokenTrade }) {
  ...
  return (
    <div className="dex-row flex items-center gap-2 px-3 py-1.5 text-[11px] font-mono border-b border-white/[0.06]">
      ...
      <span className="w-[55px] text-right flex-shrink-0 text-white/60 text-[10px]">
        {trade.marketCap > 0 ? formatCompact(trade.marketCap) : formatCompact(trade.priceUsd)}
      </span>
      <span className="flex-1 text-white/40 truncate text-[10px]">
        {shortenAddress(trade.wallet, 4)}
      </span>
      <span className="text-white/40 flex-shrink-0 text-right w-[60px] text-[10px]">
        {age}
      </span>
    </div>
  );
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-black/35 backdrop-blur overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/10 bg-white/[0.03]">
        <h3 className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Trades</h3>
      ...
      {/* Column headers */}
      <div className="flex items-center gap-2 px-3 py-1.5 text-[8px] text-white/40 uppercase tracking-widest font-bold border-b border-white/[0.06]">
      ...
      <div className="max-h-[300px] overflow-y-auto">
        {isFirstLoad ? (
          ...
          <div className="text-center text-white/40 text-[10px] py-8">No transactions found</div>
        ) : (
