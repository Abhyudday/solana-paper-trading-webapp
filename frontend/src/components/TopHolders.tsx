      ...
      {holders && holders.top10Pct > 0 && (
        <div className="px-3 py-2.5 border-b border-white/[0.06]">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[8px] text-white/40 uppercase tracking-widest font-bold">Top 10 Concentration</span>
          ...
          <div className="flex items-center justify-between mt-1.5">
            <span className="text-[8px] text-white/40 uppercase tracking-wider">Top 20</span>
            <span className="text-[10px] font-mono text-white/60">{holders.top20Pct.toFixed(1)}%</span>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center gap-1 px-3 py-2 border-b border-white/[0.06] overflow-x-auto">
          <button
            ...
            className={`px-2 py-0.5 rounded-md text-[8px] font-bold transition-all ${
              filterType === "all" ? "bg-white/10 text-white border border-white/15" : "text-white/40 hover:text-white/60"
            }`}
          >
          ...
          {activeTypes.map(([type, count]) => {
            ...
            return (
              <button
                ...
                className={`px-2 py-0.5 rounded-md text-[8px] font-bold transition-all ${
                  filterType === type ? `${cfg.bg} ${cfg.color} border ${cfg.border}` : "text-white/40 hover:text-white/60"
                }`}
              >
                {cfg.label} ({count})
              </button>
      ...
      <div className="max-h-[240px] overflow-y-auto">
        ...
        {isLoading ? (
          Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-7 skeleton-shimmer border-b border-white/[0.04] opacity-30" />
          ))
        ) : filteredHolders.length === 0 ? (
          <div className="text-center text-white/40 text-[10px] py-8">No holder data available</div>
        ) : (
          filteredHolders.map((holder, i) => {
            ...
            return (
              <div
                key={holder.address}
                className="dex-row flex items-center gap-2 px-3 py-1.5 text-[11px] font-mono border-b border-white/[0.06] group"
              >
                <span className="w-5 text-center text-[9px] text-white/40">{i + 1}</span>
                <div className="flex-1 flex items-center gap-1 min-w-0">
                  <a
                    href={`https://solscan.io/account/${holder.address}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-white/60 hover:text-[#4fc3f7] truncate text-[10px] transition-colors"
                  ...
                  >
                    {copiedAddr === holder.address ? (
                      <svg className="w-3 h-3 text-[#39FF14]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  {holder.label && (
                    <span className={`text-[7px] px-1 py-0.5 rounded-md font-bold border ${typeConfig?.bg || "bg-white/[0.06]"} ${typeConfig?.color || "text-white/40"} ${typeConfig?.border || "border-white/10"}`}>
                      {holder.label}
                    </span>
                  )}
                ...
                <span className={`w-[55px] text-right text-[10px] font-bold ${holder.percentage > 10 ? "text-accent-red" : holder.percentage > 5 ? "text-accent-yellow" : "text-white/60"}`}>
                  {holder.percentage.toFixed(2)}%
                </span>
                <span className="w-[55px] text-right text-[10px] text-white/40">
