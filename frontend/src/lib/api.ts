...
const CACHE_FRESH_MS = 5_000;   // serve cached data for 5s without any refetch
const CACHE_STALE_MS = 60_000;  // serve stale data for up to 60s while refreshing in background
...
/** Clear cached responses matching a prefix. Used after trades to force fresh portfolio data. */
export function clearResponseCache(prefix?: string): void {
  if (!prefix) { responseCache.clear(); inflight.clear(); return; }
...
export const api = {
  ...
  market: {
    ...
    getHomeTokens: () => request<{ latest: TokenInfo[]; graduating: TokenInfo[]; graduated: TokenInfo[] }>("/api/market/home"),
