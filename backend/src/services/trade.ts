...
export async function executeTrade(
  ...
  const result = await prisma.$transaction(async (tx) => {
    if (side === "buy") {
      ...
      // Clamp qty to position size to prevent floating-point-precision sell failures
      const sellQty = qty >= position.qty * 0.999999 ? position.qty : qty;
      if (position.qty < sellQty && sellQty !== position.qty) {
        throw new Error("Insufficient token balance.");
      }

      // Recalculate fee/totalCost with clamped qty
      const sellProceeds = sellQty * executionPrice;
      fee = computeFee(sellProceeds);
      totalCost = sellProceeds - fee;
      qty = sellQty;

      const realizedPnl = (executionPrice - position.avgEntryPrice) * sellQty;
      const newQty = position.qty - sellQty;
