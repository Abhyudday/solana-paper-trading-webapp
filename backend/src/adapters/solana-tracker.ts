...
const CACHE_TTL_PRICE = 30; // seconds
const CACHE_TTL_INFO = 90;
const CACHE_TTL_TOP = 120;
const CACHE_TTL_CHART = 60;
...
function rangeToSeconds(range: ChartRange): number {
  switch (range) {
    case "1s": return 1800;      // 30 min window (~1800 candles)
    case "5s": return 3600;      // 1h window (~720 candles)
    case "15s": return 7200;     // 2h window (~480 candles)
    case "30s": return 14400;    // 4h window (~480 candles)
    case "1m": return 28800;     // 8h window (~480 candles)
    case "5m": return 86400;     // 24h window (~288 candles)
    case "15m": return 172800;   // 48h window (~192 candles)
    case "30m": return 345600;   // 4d window (~192 candles)
    case "1h": return 604800;    // 7d window (~168 candles)
    case "6h": return 2592000;   // 30d window (~120 candles)
    case "1d": return 7776000;   // 90d window (~90 candles)
    case "7d": return 15552000;  // 180d window (~26 candles)
    case "30d": return 63072000; // 2yr window (~24 candles)
  }
}
...
export class SolanaTrackerAdapter implements MarketDataAdapter {
  ...
  async getTokenInfo(mint: string): Promise<TokenInfo | null> {
    ...
    try {
      const [data, dexPaid] = await Promise.all([
        deduplicatedFetch(`tokenInfo:${mint}`, () => fetchApi<{
          ...
          pools?: Array<{
            ...
            volume?: { h24?: number };
            txns?: { volume?: number; volume24h?: number };
          }>;
          supply?: number;
        }>(`/tokens/${mint}`)),
        checkDexPaid(mint),
      ...
      const info: TokenInfo = {
        ...
        volume24h: pool?.txns?.volume24h || pool?.txns?.volume || pool?.volume?.h24 || 0,
  ...
  async getTopTokens(limit: number): Promise<TokenInfo[]> {
    ...
    try {
      ...
      memCache.set("market:top", tokens, 60);
  ...
  async getLatestTokens(limit: number): Promise<TokenInfo[]> {
    ...
    if (cached) {
      const parsed = JSON.parse(cached) as TokenInfo[];
      memCache.set(cacheKey, parsed, 40);
      return parsed.slice(0, limit);
    }
    ...
    try {
      ...
      memCache.set(cacheKey, tokens, 40);
      await safeSet(cacheKey, JSON.stringify(tokens), "EX", 90);
      return tokens;
    } catch {
      ...
      tokens = items
        ...
        .map((item) => {
          ...
          return {
            ...
            image: item.token?.image,
            volume24h: pool?.txns?.volume24h || pool?.txns?.volume || pool?.volume?.h24 || 0,
            createdAt: item.createdAt || pool?.createdAt,
          };
        });
      // Sort by creation time descending (newest first, then older in sequence)
